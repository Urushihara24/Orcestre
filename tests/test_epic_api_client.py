...
class TestEpicApiClient(unittest.TestCase):
    ...
    def test_set_profile_privacy_level_success(self):
        ...
        def _req(method, url, **kwargs):
            ...
            if method == "GET":
                ...
                return True, {"acceptInvites": "private", "mutualPrivacy": "ALL"}, "ok"
        ...
        with mock.patch.object(client, "_with_token_retry", side_effect=passthrough):
            with mock.patch.object(client, "_make_request", side_effect=_req):
                ...
                self.assertEqual((calls[1][2] or {}).get("acceptInvites"), "private")
                self.assertEqual((calls[1][2] or {}).get("mutualPrivacy"), "ALL")
                self.assertEqual((r.data or {}).get("effective_level"), "PRIVATE")

    def test_set_profile_privacy_level_friends_only_limited(self):
        import epic_api_client as c
        ...
        def _req(method, url, **kwargs):
            ...
            if method == "GET":
                if len(calls) == 1:
                    return True, {"acceptInvites": "public", "mutualPrivacy": "ALL"}, "ok"
                return True, {"acceptInvites": "private", "mutualPrivacy": "ALL"}, "ok"
            return True, {"acceptInvites": "private"}, "ok"

        with mock.patch.object(client, "_with_token_retry", side_effect=passthrough):
            with mock.patch.object(client, "_make_request", side_effect=_req):
                r = client.set_profile_privacy_level("FRIENDS_ONLY")
                self.assertTrue(r.ok)
                self.assertEqual(r.code, "privacy_settings_updated_limited")
                self.assertEqual((calls[1][2] or {}).get("acceptInvites"), "private")
    ...
    def test_set_profile_privacy_level_detects_not_applied(self):
        import epic_api_client as c
        ...
        def _req(method, url, **kwargs):
            calls.append((method, url, kwargs.get("json")))
            if method == "GET":
                # Both reads return public => private request not applied.
                return True, {"acceptInvites": "public", "mutualPrivacy": "ALL"}, "ok"
            return True, {"acceptInvites": "public"}, "ok"
        ...
        with mock.patch.object(client, "_with_token_retry", side_effect=passthrough):
            with mock.patch.object(client, "_make_request", side_effect=_req):
                r = client.set_profile_privacy_level("PRIVATE")
                self.assertFalse(r.ok)
                self.assertEqual(r.code, "privacy_not_applied")
